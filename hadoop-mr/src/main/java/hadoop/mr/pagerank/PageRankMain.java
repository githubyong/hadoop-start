...
public final class PageRankMain {
    /**
    ...
    //args:hadoop-mr\src\main\java\hadoop\mr\pagerank\input\grah.txt output/
     ...
     * D  0.25  B  C
     * <p>
     *每次把权重平均分给出节点，然后重新计算权重 newpagerank  = (1- α)/总数 + α*(∑pg)
     *
     * @param file
     * @param targetFile
     * @return
    ...
    public static int createInputFile(Path file, Path targetFile) throws IOException {
        ...
        while (iter.hasNext()) {
            ...
            PageRankNode node = new PageRankNode().setPageRank(initialPageRank)
    ...
    public static double calcPageRank(Path inputPath, Path outputPath, int numNodes)
        ...
        double convergence = ((double) summedConvergence / PageRankReduce.CONVERGENCE_SCALING_FACTOR) / (double) numNodes;
